...
{
    public void MarineInit()
    {
        value = 3;
        name = CharacterDef.MARINE;
        imgFileName = "Marine";
        encode_name = "RM";
        imgFileNameOrig = "MarineOrig";
    }
    ...
    public override List<Char_Point> availableMovePoints(List<Char_Point> myPos, List<Char_Point> opponentPos, List<Char_Point> projectileList)
    {
        ...
        else
        {
            ...
            while (tmpPos.x >= 0)
            {
                ...
                else if (IsOpponentPlayer(candidate, myPos, opponentPos) /*&& !IsProjectile(projectileList, candidate)*/)
                {
                    available_move.Add(candidate);
                    break;
                }
