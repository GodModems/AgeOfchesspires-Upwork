  ...
  protected virtual void Start() {
      ...
      if (existingrunner.State != NetworkRunner.States.Shutdown) {
        ...
        if (RunnerPrefab == null) {
          RunnerPrefab = existingrunner;
                    Debug.Log("MA::NDS::Existing Runner Found");
        }
      }
    }
    ...
    if (StartMode == StartModes.Automatic) {
      if (TryGetSceneRef(out var sceneRef)) {
                Debug.Log("MA::NDS::Starting Auto With Mode::"+AutoStartAs);
  ...
  protected IEnumerator StartWithClients(GameMode serverMode, SceneRef sceneRef, int clientCount) {
    ...
    if (includesServerStart) {
      ...
      yield return StartClients(clientCount, serverMode, sceneRef);
            ...
            Debug.Log("NDS::MA::Starting Cient");
      yield return StartClients(clientCount, serverMode, sceneRef);
    }
  ...
  protected virtual Task InitializeNetworkRunner(NetworkRunner runner, GameMode gameMode, NetAddress address, SceneRef scene, Action<NetworkRunner> initialized) {
    ...
    if (sceneManager == null) {
      ...
      sceneManager = runner.gameObject.AddComponent<NetworkSceneManagerDefault>();
       ...
       /* string roomNameVal = "";
        int currentMultiplayerGameMode = UIManager.Instance.currentMultiplayerGameMode;
        if (currentMultiplayerGameMode == 5)
        {
            roomNameVal = ClientInfo.LobbyName;
        }
        else if (currentMultiplayerGameMode == 4)
        {
            roomNameVal = ServerInfo.LobbyName;
        }*/
